...
export default function EditProfileScreen() {
  ...
  const handleBioChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (words <= 150 || text.length < bio.length) {
      setBio(text);
    }
  };
    ...
    return (
      <div className="space-y-1.5 group">
        ...
        <div className="relative">
          {!isBio && <div className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400">
          ...
          {isBio ? (
            <textarea 
              ...
              disabled={!isEditing}
              className={`w-full bg-white border rounded-2xl py-4 px-4 text-base transition-all min-h-[120px] resize-none ${
      ...
      <div className="max-w-lg mx-auto px-6 py-4">
        ...
        <div className="space-y-4">
          <div className="bg-zinc-50/50 p-5 rounded-2xl border border-zinc-100 space-y-4">
            ...
            {renderField('photoURL', 'Profile Image URL', photoURL, setPhotoURL, Camera)}
            ...
            <div className="space-y-1.5 group">
              <div className="flex items-center justify-between px-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-[9px] font-bold ${countWords(bio) >= 150 ? 'text-red-500' : 'text-zinc-400'}`}>
                    {countWords(bio)}/150 words
                  </span>
                </div>
                <button 
                  onClick={() => toggleEdit('bio')}
                  className={`p-1.5 rounded-lg transition-all ${editingField === 'bio' ? 'bg-blue-50 text-blue-600' : 'text-zinc-400 hover:bg-zinc-100 hover:text-zinc-600'}`}
                >
                  <Pencil size={14} />
                </button>
              </div>
              <div className="relative">
                <textarea 
                  ...
                  className={`w-full bg-white border rounded-xl py-3 px-4 text-base transition-all min-h-[100px] resize-none ${
                    editingField === 'bio' ? 'border-blue-500 ring-4 ring-blue-500/10 focus:outline-none' : 'border-zinc-200 opacity-70'
                  }`}
